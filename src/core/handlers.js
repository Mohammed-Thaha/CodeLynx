...
class CodeLynxHandlers {
  constructor(context) {
    ...
    if (!this.usageStats) {
      this.usageStats = {
        totalRequests: 0,
        dailyRequests: 0,
        lastResetDate: new Date().toISOString().split("T")[0], // Track the date for daily limits
        models: {},
        tokens: {
          prompt: 0,
          completion: 0,
  ...
  checkDailyReset() {
    ...
    if (this.usageStats.lastResetDate !== today) {
      this.log("Resetting daily API request counters", "info");
  ...
  loadUsageStats() {
    try {
      const storageKey = "codelynx.usageStats";
      const stats = this.context.globalState.get(storageKey);
      return stats;
    } catch (error) {
      this.log(`Error loading usage stats: ${error.message}`, "error");
      return null;
    }
  }
  ...
  saveUsageStats() {
    try {
      const storageKey = "codelynx.usageStats";
      this.context.globalState.update(storageKey, this.usageStats);
    } catch (error) {
      this.log(`Error saving usage stats: ${error.message}`, "error");
    }
  }
   ...
   */
  getUsageStats() {
    // Make sure we reset daily counters if necessary
    this.checkDailyReset();
  ...
  resetDailyStats() {
    this.usageStats.dailyRequests = 0;
    this.usageStats.lastResetDate = new Date().toISOString().split("T")[0];
    this.saveUsageStats();
    this.log("Daily usage statistics manually reset", "info");
  }

  /**
   * Check if API request limits have been exceeded
   * @returns {Object} Limit status and message if exceeded
   */
  checkRequestLimits() {
    const config = vscode.workspace.getConfiguration("codelynx");
    const dailyLimit = config.get("apiDailyLimit", 100); // Default to 100 requests per day

    // Check daily limit
    if (this.usageStats.dailyRequests >= dailyLimit) {
      return {
        limited: true,
        message: `Daily API request limit (${dailyLimit}) reached. Please try again tomorrow or adjust your limit in settings.`,
      };
    }
   ...
   */
  trackApiUsage(model, usage = null) {
    ...
    if (model) {
      if (!this.usageStats.models[model]) {
        this.usageStats.models[model] = 0;
      }
      this.usageStats.models[model]++;
    }
    ...
    if (usage) {
      ...
      this.usageStats.tokens.prompt += promptTokens;
      this.usageStats.tokens.completion += completionTokens;
      this.usageStats.tokens.total += totalTokens;
    }
  ...
  async sendChatMessage(
    ...
    try {
      ...
      // Check if we've hit API request limits
      const limitStatus = this.checkRequestLimits();
      ...
      const messages = [
        {
          role: "system",
          content:
            'You are CodeLynx, an AI assistant specialized in helping developers with coding and technology. Your purpose is to assist with code review, explanation, debugging, and improvement suggestions. You are knowledgeable about multiple programming languages, frameworks, and best practices.\\n\\nYour responses should be strictly related to coding and technology. If a user asks a question that is not related to these topics, you must respond with: "The question you are asking is not in my mind." Do not answer any questions outside of your designated scope.',
        },
        ...conversationHistory,
        {
          role: "user",
      ...
      // Track API usage
      this.trackApiUsage(model, response.usage);
  ...
  getAvailableModels() {
    return [
      ...
      },
      {
        id: "llama3.1-70b",
        name: "Llama 3.1 70B",
        description: "Best for general conversations",
      },
      {
        id: "llama3.1-405b",
        name: "Llama 3.1 405B",
  ...
  async generateTests(panel, codeContent, fileName, testType) {
    try {
      ...
      switch (testType) {
        case "unit":
          systemPrompt =
            "You are an expert software engineer specializing in writing comprehensive unit tests. Generate complete, well-documented unit tests that cover edge cases, error handling, and various scenarios.";
        ...
        case "integration":
          systemPrompt =
            "You are an expert software engineer specializing in writing integration tests. Generate tests that verify how different parts of the system work together.";
        ...
        case "security":
          systemPrompt =
            "You are a cybersecurity expert specializing in writing security tests. Generate tests that verify security measures and identify potential vulnerabilities.";
      ...
      const testCode =
        response.choices[0]?.message?.content ||
        "Sorry, I could not generate tests.";
...
Provide a detailed analysis with specific line references where possible.`;
      ...
      const scanResult =
        response.choices[0]?.message?.content ||
        "Sorry, I could not analyze the code for vulnerabilities.";
